...
- 修改 idle Executor AgentClass：下一次 attempt 生效。
...
A2A Adapter 只负责 transport mapping、probe、stream/poll、cancel、artifact reference 和 failure normalization。
...
- 只支持明确的一次迁移；
- 不长期双读；
- migration 完成后旧位置变为 generated artifact 或停止读取；
- 模糊配置必须 fail closed。
...
- Kernel admission 校验；
- receipt/decision 记录 configuration revision。
...
- 删除 name allowlist；
- Codex/Pi 行为回归；
- 为 A2A 留稳定 adapter seam。

### Phase 5：Server 管理面收敛
...
- config revision 在 Planner run、decision 和 receipt 中一致；
...
- Planner、Kernel、Runtime 使用相同 configuration revision。
...
- 如引入 A2A，新增 Executor transport ADR；
...
- 强制 Docker；
- 第一阶段直接开放动态远程 Executor discovery；
