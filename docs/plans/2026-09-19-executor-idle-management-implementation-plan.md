...
- 计划日期：2026-09-19
- 状态：非 CLI 修复已实现；本地回归与浏览器验收已执行，完整发布验收尚未关闭
- 配套设计：[执行助手空闲态管理设计](2026-09-19-executor-idle-management-design.md)
- 基线参考提交：`3d87c2e`；实施时重新检查工作树与最新代码
- 当前交付：配置热增删改启停、实时助手名单、Server 写入门禁、Web 管理闭环及回归测试
- 本轮实现日期：2026-09-19；CLI 由用户明确延期，任务 6 的 CLI 部分不在本轮范围内
- 收尾提交：未创建提交；不将尚未执行的 native/Docker 验收标为完成

## 本轮修复交付记录

按用户要求保留 CLI 现状。CLI 配置命令仍有直接文件写入路径，不能宣称所有入口均已统一；
后续 CLI 重构前，不要在 Server 运行期间使用 CLI 修改配置。

本轮落实：

- 助手名单改为读取当前配置；合法 Pi/Codex 助手增删及已有权限引用变更可热生效。
- 名称/模型编辑保留原能力；权限模板及执行前权限解析使用真实 `profileId`，
  执行时只读取已授权的精确 revision，缺失或不匹配则拒绝；Auto 目标和约束完整保留。
- 共享工具配置不再包含内置助手默认模型；探测仅要求实际启用助手使用的工具。
- 职责说明编译、共享 Key 写入纳入事务；读取 Key 状态不再隐式导入。
- 嵌套激活须属于同一异步事务；补偿失败保持账户阻塞；通知失败不伪报保存失败。
- 激活异常按实际生效指针恢复，切换前的渲染失败不会误锁账户；未知指针或恢复失败仍阻塞。
- 新增受认证的助手目录与 prepare API，复用现有激活及说明预览流程。
- Web 新增/编辑/启停/删除，确认后热生效；保留其他助手未保存的编辑及失败时表单输入，
  启停也保留当前助手尚未保存的名称和模型选择。
- 停用助手可预览；工具安装与助手身份分离；无启用助手时拒绝新工作，设置和历史保留。
- `/status`、任务查询与取消类控制命令在配置事务期间仍可用。

已执行验证：

- `npm run lint`、Web TypeScript 检查及 `npm run build --prefix web`。
- 配置、路由、账户、执行器、执行绑定、Gateway、Management/Web、架构与集成最终回归：
  81 个文件、596 项通过。
- 执行链专项 `tests/execution/subtask-attempt-runner.test.ts`：28 项通过，
  覆盖权限别名、执行完成、失败清理、授权绑定、工作区及产物交付。
- 补充架构、Gateway、工具就绪和事务回归：6 个文件、52 项通过（与上项有重叠，不相加）。
- `RUN_BROWSER_E2E=1 npm test -- tests/e2e/settings-workbench-browser.test.ts`：3 项通过，
  包含新增/改名/停用/启用/删除、启停保留未保存名称和模型以及 390px 窄屏忙碌禁用。
- `tests/integration/executor-idle-management.integration.test.ts`：真实配置仓库、渲染器、
  协调器、助手查询和生产绑定组合测试通过；同工具多模型、启停、忙碌拒绝、全删除和磁盘重读。

验证边界：浏览器使用隔离 API fixture；集成测试不启动完整生产 Server，也不调用真实模型。
未执行真实 Pi/Codex 业务 smoke、生产 Server 全链路故障注入和 Docker 全量回归。
上述发布门禁、CLI 重构及最终提交记录仍保留待办，不以局部测试代替。
...
- [x] 严格空闲定义得到确认，并更新 ADR-0033。
- [ ] 五种助手写操作在整个账户忙碌时均拒绝。
- [ ] 排队、恢复、取消收尾和已接收请求没有门禁空窗。
- [x] 新工作与配置事务有对称互斥，而不只是 UI 禁用。
- [x] 用户只选择 Pi/Codex 和已有权限，没有新工具/插件范围膨胀。
- [ ] Web、CLI、完整激活、回滚、共享凭据写入共用 Server 保护。
- [ ] 新增与启用后，规划目录、执行检查和真实模型绑定立即一致。
- [x] 名称不参与判断 Pi/Codex 类型。
- [x] 同工具多助手隔离，删除内置名字不破坏其他助手。
- [x] 不新增旧任务跨配置继续执行机制，也不破坏历史审计与安全检查。
- [x] 失败补偿及故障阻塞有行为测试。
