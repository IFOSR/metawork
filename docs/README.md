...
## Active Delivery
- [Pi TUI system command and AI task separation design](plans/2026-08-28-pi-tui-system-command-separation-design.md): approved minimal correction that keeps the existing `pi-tui`, classifies every non-`user_message` Gateway operation as a compact system command, reserves the six-stage lifecycle for real AI turns, removes unrelated background-work waits, and fixes the reproduced Enter, viewport, status, focus, replay, and user-facing presentation bugs without changing Web behavior. The [implementation plan](plans/2026-08-28-pi-tui-system-command-separation-implementation-plan.md) defines the TDD sequence and real Server/TUI/Web acceptance gates.
