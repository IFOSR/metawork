...
- [Optional container compatibility validation](current/technical-overview.md#optional-container-compatibility-validation): the checked-in runtime can still build MetaClaw and the sibling AnyFusion-Pi Planner into one Node 22.19+ image for Linux compatibility checks; native macOS installation does not require Docker.
