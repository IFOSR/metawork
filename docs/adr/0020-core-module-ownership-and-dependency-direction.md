...
- ADR-0011 的单活跃顶层 Task 产品约束继续有效，但 Session `TaskAdmissionGate` 只是当前实现；最终 admission 决策归 Control Kernel。
...
- ADR-0014 的 PlanningAgent/PolicyKernel/Runtime 主链继续有效；`direct_reply` 专用 Kernel 入口是迁移期例外，Phase 3 目标为统一 `decide` seam。
