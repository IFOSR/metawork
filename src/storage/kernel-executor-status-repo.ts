...
export class KernelExecutorStatusRepo {
  ...
  upsert(projection: KernelExecutorStatusProjection): void {
    ...
    `).run(projection.agentClassName, projection.classHealth, JSON.stringify(projection.recentAttempts), projection.updatedAt);
  }
}
...
function rowToProjection(row: StatusRow): KernelExecutorStatusProjection {
  ...
  return { agentClassName: row.agent_class_name, classHealth: row.class_health, recentAttempts, updatedAt: row.updated_at };
}
