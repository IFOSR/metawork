...
persist_ssh_environment() {
  ...
  grep -vE "^(OPENAI_API_KEY|OPENAI_BASE_URL|ANTHROPIC_API_KEY|ANTHROPIC_BASE_URL|DEEPSEEK_API_KEY|GOOGLE_GENERATIVE_AI_API_KEY|OPENROUTER_API_KEY|PI_SKIP_VERSION_CHECK|PI_TELEMETRY|METACLAW_HOME|METACLAW_PLANNER_CODEX_HOME|METACLAW_EXECUTOR_CODEX_HOME|METACLAW_PLANNER_SCHEMA_PATH|METACLAW_PLANNER_WORKDIR|METACLAW_PLANNER_ENV_FILE|METACLAW_PLANNER_CODEX_COMMAND|METACLAW_PLANNER_TUI_COMMAND|METACLAW_PLANNER_TUI_SOCKET|ANYFUSION_BRIDGE_SOCKET|ANYFUSION_PLANNER_MODE|ANYFUSION_PLANNER_SCHEMA_PATH|METACLAW_CODEX_EXECUTOR_ENV_FILE|METACLAW_PI_EXECUTOR_ENV_FILE|METACLAW_CONTROL_NETWORK|METACLAW_CONTROL_HOST|METACLAW_DOCKER_HOST_PATH_MAP)=" "$environment_file" > "$tmp" 2>/dev/null || true
  for kv in \
    ...
    "METACLAW_PLANNER_CODEX_COMMAND=${METACLAW_PLANNER_CODEX_COMMAND:-/opt/anyfusion-codex/bin/codex}" \
    "METACLAW_PLANNER_TUI_COMMAND=${METACLAW_PLANNER_TUI_COMMAND:-/opt/anyfusion-codex/bin/codex}" \
    ...
    "ANYFUSION_PLANNER_SCHEMA_PATH=${ANYFUSION_PLANNER_SCHEMA_PATH:-/opt/metaclaw/schema/planning-agent-plan-v6.schema.json}" \
