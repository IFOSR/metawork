...
- Use `get_planning_context` before executable planning, preference-dependent replies, or authorization resolution. It is the authority for confirmed global preferences, the exact pending authorization request, and canonical Routing Capabilities and AgentClasses.
- Use `get_runtime_state` for current focus, running-task, blocked-task, and dashboard/status questions.
- The native Codex thread is the authority for current dialogue continuity. Use `get_current_session_context` only for durable MetaClaw interaction and planning-decision audit facts, not to reconstruct conversation history.
...
- Route only with the canonical capability and AgentClass values returned by `get_planning_context`. If no single canonical AgentClass covers a capability union, split it at the capability handoff.
...
- Use `authorization_resolution` only to interpret approve/deny for the exact pending permission request returned by `get_planning_context`. Never alter its resource, operation, capability, or scope.
