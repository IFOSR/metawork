import {
  DockerCliAttemptSandboxAdapter,
  type DockerCommandRunner,
} from '../../src/execution/docker-cli-attempt-sandbox-adapter.js';
...
describe('DockerCliAttemptSandboxAdapter', () => {
  test('creates a constrained per-attempt container without Docker endpoints', async () => {
    const runner = new FakeRunner();
    const adapter = new DockerCliAttemptSandboxAdapter(runner);
    await adapter.create({
      ...
      environment: {}, controlNetwork: 'metaclaw-control', egressMode: 'disabled', limits: DEFAULT_ATTEMPT_SANDBOX_LIMITS,
      mounts: [
  ...
  test('fails closed on image drift and writable source mounts', async () => {
    ...
    const base = {
      ...
      controlNetwork: 'metaclaw-control', egressMode: 'disabled' as const, limits: DEFAULT_ATTEMPT_SANDBOX_LIMITS,
      mounts: [{ source: '/workspace', target: '/workspace', mode: 'rw' as const }],
    };
  ...
  test('translates containerized control-plane paths to Engine-visible sibling mount sources', async () => {
    ...
    await adapter.create({
      ...
      controlNetwork: 'metaclaw-control', egressMode: 'disabled', limits: DEFAULT_ATTEMPT_SANDBOX_LIMITS,
