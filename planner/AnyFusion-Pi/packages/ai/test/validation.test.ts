...
describe("validateToolArguments", () => {
	...
	it("preserves null when a serialized union explicitly accepts null", () => {
		const schema = {
			anyOf: [{ type: "string" }, { type: "null" }],
		} as Tool["parameters"];
		const { tool, toolCall } = createToolCallWithPlainSchema(schema, null);

		expect(validateToolArguments(tool, toolCall)).toEqual({ value: null });
	});

	it("preserves strings when a serialized union explicitly accepts strings", () => {
		const schema = {
			anyOf: [{ type: "number" }, { type: "string" }],
		} as Tool["parameters"];
		const { tool, toolCall } = createToolCallWithPlainSchema(schema, "42");

		expect(validateToolArguments(tool, toolCall)).toEqual({ value: "42" });
	});

	it("falls back to ordered conversion when no union member matches the runtime type", () => {
		const schema = {
			anyOf: [{ type: "number" }, { type: "boolean" }],
		} as Tool["parameters"];
		const { tool, toolCall } = createToolCallWithPlainSchema(schema, "1");

		expect(validateToolArguments(tool, toolCall)).toEqual({ value: 1 });
	});
