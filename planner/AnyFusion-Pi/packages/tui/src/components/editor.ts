		...
		if (this.autocompleteState && this.autocompleteList) {
			...
			if (kb.matches(data, "tui.input.tab")) {
				...
				if (selected && this.autocompleteProvider) {
					...
					const shouldContinue = this.autocompleteProvider.shouldContinueCompletion?.(selected, result) ?? false;
					this.state.lines = result.lines;
					this.state.cursorLine = result.cursorLine;
					this.setCursorCol(result.cursorCol);
					this.cancelAutocomplete();
					if (this.onChange) this.onChange(this.getText());
					if (shouldContinue) this.tryTriggerAutocomplete();
				}
				return;
			}
			...
			if (kb.matches(data, "tui.select.confirm")) {
				...
				if (selected && this.autocompleteProvider) {
					...
					const shouldContinue = this.autocompleteProvider.shouldContinueCompletion?.(selected, result) ?? false;
					const isSlashCompletion = this.autocompletePrefix.startsWith("/");
					this.state.lines = result.lines;
					this.state.cursorLine = result.cursorLine;
					this.setCursorCol(result.cursorCol);
					this.cancelAutocomplete();
					...
					// Fall through to submit completed slash commands.
				}
			}
		}
