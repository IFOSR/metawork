...
describe("Editor component", () => {
	...
	describe("Autocomplete", () => {
		...
		it("continues autocomplete after Tab applies an item that opens a nested completion", async () => {
			const editor = new Editor(createTestTUI(), defaultEditorTheme);
			...
			const mockProvider: AutocompleteProvider = {
				...
				applyCompletion,
				shouldContinueCompletion: (item) => item.value === "/task ",
			};

			editor.setAutocompleteProvider(mockProvider);
			for (const ch of "/ta") editor.handleInput(ch);
			await flushAutocomplete();
			assert.strictEqual(editor.isShowingAutocomplete(), true);
			...
			assert.strictEqual(editor.getText(), "/task ");
			assert.deepStrictEqual(requestedTexts, ["/ta", "/task "]);
			assert.strictEqual(editor.isShowingAutocomplete(), true);
			...
			assert.ok(rendered.includes("list"));
		});

		it("continues autocomplete instead of submitting when Enter accepts an incomplete command", async () => {
			const editor = new Editor(createTestTUI(), defaultEditorTheme);
			const requestedTexts: string[] = [];
			...
			const mockProvider: AutocompleteProvider = {
				getSuggestions: async (lines, _cursorLine, cursorCol) => {
					...
					requestedTexts.push(text);
					if (text === "/ta") {
						return { items: [{ value: "/task ", label: "/task" }], prefix: "/ta" };
					}
					if (text === "/task ") {
						return {
							items: [
