...
export function printHelp(_extensionFlags?: ExtensionFlag[]): void {
	console.log(`${chalk.bold("AnyFusion Planner")} - 对话、只读查询与任务规划

${chalk.bold("Usage:")}
  ${APP_NAME} [options] [message]

${chalk.bold("Options:")}
  --mode <mode>          运行模式：text、json 或 rpc
  --print, -p            处理一条消息后退出
  --continue, -c         继续最近会话
  --resume, -r           选择会话继续
...
Provider、Model、凭据、工具和扩展由 AnyFusion 固定管理。
Planner 不执行 shell、不修改项目文件，也不负责任务执行、恢复或 Git 发布。`);
}
