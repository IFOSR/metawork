...
export interface MainOptions {
	/** External factories are accepted only for API compatibility and are never loaded by AnyFusion Planner. */
	extensionFactories?: ExtensionFactory[];
}

export async function main(args: string[], _options?: MainOptions) {
	resetTimings();
	process.env.PI_OFFLINE = "1";
	process.env.PI_SKIP_VERSION_CHECK = "1";
	process.env.PI_TELEMETRY = "0";
	const plannerPolicyErrors = validatePlannerInvocation(args);
	if (plannerPolicyErrors.length > 0) {
		for (const message of plannerPolicyErrors) console.error(chalk.red(`Error: ${message}`));
		process.exitCode = 1;
		return;
	}
	...
	const plannerBootstrap = isRuntimeMetadataCommand(parsed)
		? undefined
		: createAnyFusionPlannerBootstrap({
				cwd: sessionCwd,
				schemaPath: process.env.ANYFUSION_PLANNER_SCHEMA_PATH,
			});
	if (plannerBootstrap) parsed.systemPrompt = plannerBootstrap.systemPrompt;
	...
	const createRuntime: CreateAgentSessionRuntimeFactory = async ({
		...
		const services = await createAgentSessionServices({
			...
			resourceLoaderOptions: {
				...
				extensionFactories: plannerBootstrap?.extensionFactories,
			},
		});
		...
		const created = await createAgentSessionFromServices({
			...
			thinkingLevel: plannerBootstrap?.thinkingLevelOverride ?? sessionOptions.thinkingLevel,
			scopedModels: sessionOptions.scopedModels,
			tools: plannerBootstrap?.activeToolNames ?? sessionOptions.tools,
			excludeTools: sessionOptions.excludeTools,
			noTools: sessionOptions.noTools,
			customTools: [...(sessionOptions.customTools ?? []), ...(plannerBootstrap?.customTools ?? [])],
		});
