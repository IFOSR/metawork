...
- Added the responsive read-only AnyFusion Task dashboard with bounded MetaClaw Task/Subtask/Executor projections and narrow-terminal degradation.
- Added transient Host-projected permission review through Pi's native approve/deny Selector, including expiry, snooze, closure, disconnect cleanup, and capped reconnect handling without adding permission facts to the conversation.
- Added the AnyFusion Executor registration window, including known Codex, Pi, and Hermes profiles, generic CLI session bindings, absolute path and environment-source confirmation, and submission through the shared MetaClaw command boundary.
- Added Executor enablement, verification, driver, and latest smoke-audit projections to the AnyFusion Task dashboard.

### Fixed

- Fixed the Planner MCP child process to start in the Runtime-authorized workspace instead of the hardcoded `/workspace` path, unblocking native macOS Planner sessions.
- Restored live Executor activity in the AnyFusion Task dashboard with Pi's native Loader and the current MetaClaw Session projection.
- Restored integrated Subtask result visibility as persistent passive conversation messages, including Executor reports and complete artifact path lists, without triggering or steering a Planner turn.
