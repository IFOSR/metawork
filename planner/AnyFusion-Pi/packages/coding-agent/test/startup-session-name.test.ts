...
describe("startup session name", () => {
	it("rejects managed model selection before mutating the selected session", async () => {
		const dirs = setup();
		...
		expect(result.stderr).toContain("--model is managed by AnyFusion");
	...
	it("rejects empty --name values without appending session metadata", async () => {
		const dirs = setup();
		const result = await runCli(["--session", dirs.sessionFile, "--name", "   ", "-p", "hi"], dirs);
