...
async function runCli(args: string[]): Promise<{ stdout: string; stderr: string; code: number | null }> {
	...
	const projectConfigDir = join(projectDir, ".anyfusion");
...
describe("stdout cleanliness in non-interactive modes", () => {
	...
	it("keeps stdout empty when a managed trust flag is rejected in JSON mode", async () => {
		const result = await runCli(["--mode", "json", "--help", "--approve"]);

		expect(result.code).toBe(1);
		expect(result.stdout).toBe("");
		expect(result.stderr).toContain("--approve is managed by AnyFusion");
	});

	it("keeps stdout empty when a managed trust flag is rejected in print mode", async () => {
