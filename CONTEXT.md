...
The default local surface is the pinned `MetaAny/anyfusion-codex` downstream native TUI. `src/tui-bridge/` launches that Planner-only binary with the isolated Planner `CODEX_HOME` and exposes a local mode-`0600` Unix JSONL socket. The Task panel receives only a Session projection. A Codex Stop hook may hand one structured Planner proposal to `MetaclawSession`, which reruns the existing v6 schema and semantic validation before emitting the existing `plan_proposed` event into `DurableKernelWorkflow`; the bridge has no direct database, Kernel, scheduler, or Executor dependency. Planner MCP remains read-only. Executor attempts continue to use stock Codex from the canonical attempt image. The original Ink implementation under `src/tui/` remains intact as an explicitly unmaintained standby module selected with `METACLAW_STANDBY_TUI=1`.
