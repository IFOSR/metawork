...
`src/planning/` owns the PlanningAgent interface (`AnyFusionPlanningAgent`), controlled-lifecycle AnyFusion-Pi JSONL RPC runner, minimal planning context, strict v6 structured output, and catalog-aware validation. One live MetaClaw session maps to one persisted Pi session file. Non-interactive surfaces use `--mode rpc` over stdin/stdout JSONL and serialize writers per session; MetaClaw does not replay SQLite interaction history into prompts. Stable instructions and the read-only Planner boundary are fixed in the AnyFusion-Pi fork, while MetaClaw remains the only v6 validator and the only owner of Task, Kernel, Executor and storage mutation. `src/work-graph/` owns the shared v5 graph types and pure structural rules consumed by Planning, Kernel, and Execution. Planner timeout, RPC failure, or invalid output after one repair fails closed to clarification; there is no earlier-schema production parser, legacy intent route, semantic default, keyword fallback, or Codex Planner fallback.

The default local surface is the pinned sibling `AnyFusion-Pi` fork running with its self-contained Node 22 runtime. `src/tui-bridge/` launches the Planner-only binary and exposes a mode-`0600` Unix JSONL host socket. The TUI receives only Session projections, submits at most one proposal envelope for a turn, requests slash-command completion state from `MetaclawSession`, and may transport an explicit user-authored slash command; completion, validation, dynamic Task/Executor candidates, and execution still come only from the existing `CommandCatalog`. `MetaclawSession` reruns the existing v6 schema and semantic validation before emitting the existing `plan_proposed` event into `DurableKernelWorkflow`; the bridge has no direct database, Kernel, scheduler, or Executor dependency. Planner capabilities are limited to conversation and read-only query/planning; it cannot synthesize privileged commands, edit, execute shell, mutate Task state, authorize work or publish Git changes. Executor attempts continue to use their canonical Codex/Pi attempt images. The original Ink implementation under `src/tui/` remains intact as an explicitly unmaintained standby module selected with `METACLAW_STANDBY_TUI=1`.
