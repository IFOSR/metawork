...
The default local surface is the pinned sibling `AnyFusion-Pi` fork running with its self-contained Node 22 runtime. `src/tui-bridge/` launches the Planner-only binary and exposes a mode-`0600` Unix JSONL host socket. The TUI receives only Session projections and submits at most one proposal envelope for a turn. `MetaclawSession` reruns the existing v6 schema and semantic validation before emitting the existing `plan_proposed` event into `DurableKernelWorkflow`; the bridge has no direct database, Kernel, scheduler, or Executor dependency. Planner capabilities are limited to conversation and read-only query/planning; it cannot edit, execute shell, mutate Task state, authorize work or publish Git changes. Executor attempts continue to use their canonical Codex/Pi attempt images. The original Ink implementation under `src/tui/` remains intact as an explicitly unmaintained standby module selected with `METACLAW_STANDBY_TUI=1`.
